...
def unitSetup():
	global unitNumber, wavCoef, radSens, linCoefs, wavelength, wavelengthBins, wavelengthBoxcar, cieXt, cieYt, cieZt, chlAt, chlBt, nIRt, nUVt, cieWav, chlA, chlB, nIR, cieX, cieY, cieZ, nUV
	...
	if(len(wavCoef) != pixels or len(radSens) != pixels): # only load values from file the first time the code runs (to work out unit number)
		for line in open("./calibration_data.txt"):
			...
			try:
				...
				print("except (not a unit number)")
			else:
...
##			print(row)
	...
	##				print(cieWav)
					...
					nUV = readLine(row)
##			else:
##                                for i in range(0, len(visSystems)):
##                                        TODO - add ability to export cone-catch from other spectral sensitivities
...
##			nImR = ((imR/maxRGB)**0.42) * 255 * br
##			nImG = ((imG/maxRGB)**0.42) * 255 * br
##			nImB = ((imB/maxRGB)**0.42) * 255 * br
...
##			nImI = ((imI/maxIGU)**0.42) * 255 * br
##			nImGG = ((imGG/maxIGU)**0.42) * 255 * br
##			nImU = ((imU/maxIGU)**0.42) * 255 * br
...
##		plotSize = widgetDims[2] # width
##		if(widgetDims[3] < plotSize):
##			plotSize = widgetDims[3] # use height if smaller
		plotSize -= 2 ## padding
...
##        ax.imshow(imCol)
##        canvas.draw()
	#global panDim, pan, tilt, tiltDim
...
def getSpec():
	...
	global imI, imU, imGG, imChlA, imChlB, imNDVI, maxIGU, hspec, hspecPan, hspecTilt, fileImportFlag, loadPath, loadLine, lines, selX, selY, wavelengthBoxcar, stopFlag
...
##						for k in range(0, hspec.shape[2]):
##							try:
##								if(hspec[i, j, k] >= 0):
##									ts = f'{hspec[i, j, k]:.3e}'							
##							except:
##								print("Failed to parse output line; pan: " + str(hspecPan[j]) + " tilt: " + str(hspecTilt[i]))
##							dataString2 += ',' + ts
					...
					for k in range(0, boxcarN): #repeat boxcar window
						if(i+k < pixels):
							if radSens[i+k] > 0 and len(output) > loc+5 :
								#print(" " + output[loc+5] + ", " + darkVals[loc+boxedLength*j] + ", " + str(radSens[i+k]))
								#x = float(output[loc+5])-float(darkVals[loc+5+boxedLength*j]) # subtract corresponding dark value
								#x = float(output[loc+5])-250 # subtract corresponding dark value
								...
								if(x < 0):
									x = -1 * math.exp(math.log(-x)*linCoefs[0] + linCoefs[1])
										#linMultiplier = -1* linCoefs[0]*math.log((-x+1)*linCoefs[1])
##								x = (x/linMultiplier) / (radSens[i+k] * intTime * wavelengthBins[i+k])
##								x = (x/linMultiplier) / (radSens[i+k] * intTime)
...
##									print("dark:" + darkVals[loc+boxedLength*j] + " light:" + output[loc+5] + " sens:" + str(radSens[i+k]) + " x:"+ str(x))
								...
								lum += x * cieYt[i+k]
					#hspec[tilt, pan, loc-1] += xSum/boxcarN # CHECK BOXCAR shouldn't be used here
					hspec[tilt, pan, loc] += xSum
					loc += 1
				lum = lum * 683 #luminance: W/(sr*sqm*nm)
				#ts = "Scanning " +  str(float(pan + (tilt * panDim)) / float(tiltDim * panDim) + "% done")
...
##	if btPreview.config("relief")[-1] == "sunken":
##		btPreview.config(relief="raised")
##		preview = 0
##	else:
##		btPreview.config(relief="sunken")
...
##	tR = imR[tiltDim-1-selY, selX]
##	tG = imG[tiltDim-1-selY, selX]
##	tB = imB[tiltDim-1-selY, selX]
##	print("R:"+str(tR)+" G:"+str(tG)+" B:"+str(tB))
...
##				if(refs[i] != 0):
##					refs[i] = (float(reflVal)/100)/refs[i]
			with np.errstate(divide='ignore', invalid='ignore'):
				refs = (float(reflVal)/100)/refs # can't deal with zeros
					
##			m = refs
##			m = float(reflVal)/100
##			refs = np.divide(m, refs, out=np.zeros_like(m), where=refs!=0)
